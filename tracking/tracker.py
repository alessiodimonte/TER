...
class Tracker:
    ...
    def predict(self):   # This function should be called once every time step, before `update`.
    ...
    def update(self,detections):
        ...
        # 1. For matched results
        for track_idx,detection_idx in matches:     # Update the corresponding detection in tracks
            self.tracks[track_idx].update(self.kf,detections[detection_idx])
        #2. Call mark_missed for unmatched tracks
        # track mismatch, if Tantative delete; Delete the update if it takes a long time
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
        #3. Initialize the unmatched detection, detection mismatch
        for detection_idx in unmatched_detections:
            self._initiate_track(detections[detection_idx])
        #Get the latest list of tracks that are labeled as Confirmed and Tentative
        self.tracks = [t for t in self.tracks if not t.is_deleted()]
        # Update distance metric.
        active_targets = [t.track_id for t in self.tracks if t.is_confirmed()]
        features,targets=[],[]
        for track in self.tracks:
            #Get track ids for all Confirmed states
            if not track.is_confirmed():
                continue
            features += track.features
            #Get the trackID of each feature
    ...
    def _match(self,detections):
        ...
        def gated_metric(tracks,dets,track_indices,detection_indices):
            ...
            # Calculate the cost matrix by cosine distance
            cost_matrix = self.metric.distance(features,targets)
            # Calculate the cost matrix after gating
